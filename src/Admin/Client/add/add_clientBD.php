...
if (isset($_POST['add_new_client'])) {
    ...
    Votre compté à été crée. Pour valider votre compte, veuillez cliquer sur le lien ci-dessous :<br><br>
    ...
    if (isset($_POST['name']) && isset($_POST['firstname']) && isset($_POST['username']) && isset($_POST['phone']) && isset($_POST['email']) && isset($_POST['password'])) {
        ...
        if ($check_info->rowCount() > 0) {
            ...
            if ($row['cli_username'] == $username) {
                $_SESSION['erreur'] = 'le nom d\'utilisateur ' . $username . ' à déjà été utilisé';
                header("location: add_client.php");
            } else if ($row['cli_mail'] == $email) {
                $_SESSION['erreur'] = 'L\'adresse mail ' . $email . ' est déjà existante';
                header("location: add_client.php");
            }
        } else {
            ...
            if ($stmt) {
                ...
                $_SESSION['erreur'] = 'Echoue de l\'ajout du client';
                header("location: add_client.php");
                exit();
            }
        }
    } else {
        $_SESSION['erreur'] = "Toutes les données doivent être renseignées";
        header("location: add_client.php");
        exit();
    }
}
